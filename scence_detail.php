...
if (isset($_GET['id']))
		{
			$mark=0;
			if(!!($rows=_fetch_array("select * from t_photo where t_id='{$_GET['id']}'")))
			{
				$html = array();
				$html['scence_id'] = $rows['t_id']; 
				$html['name'] = $rows['t_name'];
				$html['cityname'] = $rows['t_cityname'];
				$html['url'] = $rows['t_url'];
				$html['scence_content'] = $rows['t_content'];
				$html['price'] = $rows['t_price'];
				$html['grade'] = $rows['t_grade'];
				$html['special'] = $rows['t_special'];
				$html['introduce'] = $rows['t_introduce'];
				$html['route'] = $rows['t_route'];
				$html['own_cost'] = $rows['t_own_cost'];
				$html['include_cost'] = $rows['t_include_cost'];
				$html['limit'] = $rows['t_limit'];
				$html['travel'] = $rows['t_travel'];
				$html['scence_introduce'] = $rows['t_scence_introduce'];
				$html['longitude'] = $rows['t_longitude'];
				$html['latitude'] = $rows['t_latitude'];
					if(!!($rows=_fetch_array("select * from t_grade where t_scence_id='{$_GET['id']}'")))
					{
						$grade['total_grade'] = $rows['t_total_grade'];
						$grade['describe_grade'] = $rows['t_describe_grade'];
						$grade['travel_arragement'] = $rows['t_travel_arragement'];
						$grade['good_appraise'] = $rows['t_good_appraise'];
					
						$count = _query("select count(*) as 'num'   from t_pay where t_scence_id = '1'");
						if(!!($count=_fetch_array("select count(*) as 'num'   from t_pay where t_scence_id = '{$_GET['id']}'"))){
...
<div id="scence">
	<h2>景点详细</h2><h3><?php echo $html['name']?></h3>
		...
		<p><?php echo $html['scence_content']?></p><p><?php echo $html['price']?></p>
			...
			<div id="pingfen">
				<span><?php echo $grade['total_grade'];?></span>&nbsp/5&nbsp(  &nbsp<?php echo $count['num']?>人出游)
									<p>导游讲解·····<?php echo $grade['total_grade']?></p>
									<p>描述相符·····<?php echo $grade['describe_grade']?></p>
									<p>行程安排·····<?php echo $grade['travel_arragement']?></p>
			...
			<div id ="select">
				<form method="post" action="order.php?action=order&id=<?php echo $_GET['id']?>">
					<dl><?php echo $clean['scence_id']?>
						<dd>出发时间 <select name="year"><option value="2017" selected="selected">2017</option><option value="2018">2018</option><option value="2019">2019</option><option value="2020">2020</option>
														</select>
										...
										成人 <select name="adult"><option value="1" selected="selected">1</option><option value="2" >2</option><option value="3" >3</option>
																				<option value="4" >4</option><option value="5" >5</option><option value="6" >6</option><option value="7" >7</option>
																				<option value="8" >8</option><option value="9" >9</option><option value="10" >10</option>
																				</select>	
									
										儿童半价 <select name="child"><option value="0" selected="selected">0</option><option value="1" >1</option><option value="2" >2</option><option value="3" >3</option>
										<option value="4" >4</option><option value="5" >5</option><option value="6" >6</option><option value="7" >7</option>
										<option value="8" >8</option><option value="9" >9</option><option value="10" >10</option>
										</select>		
						...
						<?php 
							if(!$rows=_fetch_array("select t_id from t_user where t_id =( select  t_business_id from t_photo where t_id = '{$html['scence_id']}')")){
			...
			<div id="mokuai">
				<a href="scence_detail?id=<?php echo $html['scence_id']?>&detail=jieshao">景点介绍</a>
				<a href="scence_detail?id=<?php echo $html['scence_id']?>&detail=jingdian">周边景点</a>
				<a href="scence_detail?id=<?php echo $html['scence_id']?>&detail=tuangou">周边团购</a>
				<a href="scence_detail?id=<?php echo $html['scence_id']?>&detail=jiaotong">附近交通</a>
				<a href="scence_detail?id=<?php echo $html['scence_id']?>&detail=hotel">附近交通</a>
				<a href="scence_detail?id=<?php echo $html['scence_id']?>&detail=comment">查看评论</a>
			</div>
			<?php if($mark == 0){?>
						<div id ="jieshao">产品特色：<?php echo $html['special'];?>
											<p>产品概要:</p>
											<p>住宿5晚···5星级酒店</p>
											<p>景点：共10个  经典·xxx·yyy·zzz</p>
											<p>用餐：9次自理+9次团餐</p>
											<p>交通：8.5小时用车</p>
						</div>
						<div id="xingcheng">详细行程：<br/>
							<?php echo $html['route'];?>
						</div>
						<div id="xuzhi">须知
										
											<p>费用包含</p>
											<?php echo $html['include_cost'];?>
											<p>自理费用</p>
											<?php echo $html['own_cost'];?>
											<dl>
												<dt>预定限制</dt>
											<dd>
											<ul><li><?php echo $html['limit'];?></li></ul></dd></dl>
						</div>				
											
			
						<div id="youji">相关游记
							<p><?php echo $html['travel']?></p>
				...
				<?php }elseif ($mark == 1){?>
						<div >
							<?php echo $html['scence_introduce']?>
						</div>
		<?php }elseif ($mark == 2){?>
		...
		<?php }elseif ($mark == 3){?>
						<div>
							...
							[城关区]拉萨空港花园酒店（周末海鲜自助晚餐） ￥178
						</div>
						<div>
							[城关区]拉萨空港花园酒店（周末休闲自助午餐） ￥78
						</div>
						<div>
							[城关区]拉萨空港花园酒店（休闲双人下午茶） ￥68
						</div>
						<div>
							[城关区]拉萨空港花园酒店（海鲜自助晚餐）￥168
						</div>
												<div >
